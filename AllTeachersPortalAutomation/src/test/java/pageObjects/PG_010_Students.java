...
public class PG_010_Students extends BaseClass{
	...
	public PG_010_Students Click_on_the_classDetail(String classDetails) {
		...
		try {
			...
			if(driver.findElement(By.xpath("//div[contains(@class,'MuiChip-colorPrimary')]"))!=null)
					{
				driver.findElement(By.xpath("(//p)[1]//following::div[3]/div/span[1][text()='Class  "+classDetails+"']")).click();
					}
	...
	public PG_010_Students Click_On_the_student(String studentid) {
		...
		String methodName = Thread.currentThread().getStackTrace()[1].getMethodName().replace("_", " ");
		try {
		 driver.findElement(By.xpath("//a[text()='"+studentid+"']")).click();
		 Thread.sleep(2000);
